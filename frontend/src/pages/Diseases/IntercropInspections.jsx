...
const IntercropInspections = () => {
  ...
  const handleDelete = async (id) => {
    try {
      const response = await axios.delete(`http://localhost:5000/api/harvest/${id}`); // Changed endpoint
      if (response.data.success) {
        fetchInspections();
      } else {
    ...
    { title: "Comments", dataIndex: "comments", key: "comments" },
    {
      ...
      render: (text, record) => (
        <span style={{ display: 'flex', gap: '10px' }}>
          <Button onClick={() => navigate(`/intercropInspections/edit/${record.inspectionId}`)}>Edit</Button>
        ...
        <div className="content" style={{ flex: 1, padding: '20px', marginLeft: '280px' }}> {/* Adjust margin-left to push content right */}
          ...
          <nav className="flex items-center justify-between p-4 bg-transparent">
            ...
            <div className="flex space-x-4">
              ...
              <Link to="/IntercropInspections" className="text-[#236A64] font-semibold">
                Inspections
              </Link>
              <Link to="/IntercropTreatments" className="text-[#3CCD65] hover:text-[#2b8f57]">
                Treatments
              </Link>
          ...
          <div className="mt-4"> {/* Adjusted margin */}
            ...
            <div className="flex justify-center items-center">
              <h1 className="text-5xl font-semibold">Intercrop Inspections and Disease Identification</h1>
            </div>
