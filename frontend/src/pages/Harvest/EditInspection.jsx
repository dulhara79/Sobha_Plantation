...
const EditInspection = () => {
  ...
  const validatequalityController = (_, value) => {
    ...
    if (!/^[A-Z][a-z\s]*$/.test(value)) {
      return Promise.reject(new Error('Quality Controller must start with an uppercase letter and only contain lowercase letters and spaces.'));
    }
  ...
  useEffect(() => {
    const fetchInspectionReport = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/api/quality/${id}`);
        const { data } = response;
        form.setFieldsValue({
          ...data,
      ...
      <div className="w-full max-w-lg p-6 bg-white rounded-lg shadow-lg">
        ...
        >
          ...
          <Form.Item
            name="checkDate"
            label="Check Date"
            rules={[{ required: true, message: 'Please select the checkDate!' }]}
          >
            <DatePicker
              format="YYYY-MM-DD"
